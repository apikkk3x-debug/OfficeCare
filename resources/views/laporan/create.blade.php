...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex justify-between items-center">
        ...
        <a href="{{ route('karyawan.dashboard') }}" class="text-slate-600 hover:text-slate-800 text-sm font-semibold border px-3 py-1.5 rounded-lg transition">
            ← Kembali
        </a>
    </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
        <form action="{{ route('laporan.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <select name="id_barang" id="select_barang" onchange="cekBarangBaru(this)" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-sm">
                    ...
                    @endforeach
                    <option value="tambah_baru" class="font-bold text-blue-600">+ Tambah Barang Baru...</option>
                </select>
            </div>
            ...
            <div id="form_barang_baru" class="hidden p-4 bg-slate-50 rounded-lg border border-slate-200 mb-4 space-y-3">
                <h4 class="text-xs font-bold uppercase tracking-wider text-blue-600">Form Tambah Barang Baru</h4>
                <div>
                    <label class="block text-slate-700 text-xs font-semibold mb-1">Nama Barang Baru</label>
                    <input type="text" name="nama_barang_baru" placeholder="Contoh: AC LG 2 PK" class="w-full px-3 py-2 border rounded-lg text-sm bg-white">
                </div>
                <div>
                    <label class="block text-slate-700 text-xs font-semibold mb-1">Lokasi / Ruangan</label>
                    <input type="text" name="lokasi_baru" placeholder="Contoh: Ruang Meeting B" class="w-full px-3 py-2 border rounded-lg text-sm bg-white">
                </div>
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="deskripsi_kerusakan" rows="3" required placeholder="Jelaskan kerusakannya..." class="w-full px-3 py-2 border rounded-lg text-sm"></textarea>
            </div>

            <div class="flex justify-end space-x-3">
                <a href="{{ route('karyawan.dashboard') }}" class="bg-slate-100 text-slate-600 px-4 py-2 rounded-lg text-sm font-semibold">Batal</a>
                <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-lg text-sm font-semibold hover:bg-blue-700">Kirim Laporan</button>
            </div>
        </form>
    </div>
</div>
